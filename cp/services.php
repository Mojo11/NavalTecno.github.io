    ...
    if($_FILES['image']['error'] == '0'){
        ...
        if (in_array($ext, $extension) == TRUE) {
            if (move_uploaded_file($temp, $UploadFolder . "/" . $name)) {
                ...
                if($crud->saveRecords('sf_services', $data))
                {
                    $sucess=1;
                } else {
                    $sucess=5;
...
if (isset($_POST['edit_content'])) {
    if ($_FILES['image']['size'] == 0) {
        ...
        $data['name']        =   addslashes($_POST['name']);
        $data['description'] =   addslashes($_POST['description']);
        $update = $crud->dbRowUpdate('sf_services', $data, 'id', $_REQUEST['sid']);
        $sucess=4;
    } 
    else {
            $errors = '';
...
if($sucess==1){ ?>
    <script>alert('Service added successfully'); window.location.replace("services.php");</script>
...
<?php } else if($sucess==4){ ?>
    <script>alert('Service updated successfully'); window.location.replace("services.php");</script>
<?php } else if($sucess==5){ ?>
    <script>alert('Unable to save service'); window.location.replace("services.php");</script>
<?php }

if(isset($_REQUEST['did']) && !empty($_REQUEST['did'])){
    $crud->delete($_REQUEST['did'], 'sf_services');
    $image_name = $_REQUEST['image'];
    unlink("../upload/services/$image_name"); ?>
    <script>alert('Service deleted successfully'); window.location.replace("services.php");</script>
<?php
}

$services = $crud->read('sf_services', array(), array(), 'order by id desc', '');
if(isset($_REQUEST['sid']) && !empty($_REQUEST['sid'])){
    $service = $crud->singleRead('sf_services', array(), 'id', $_REQUEST['sid']);
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
        <meta charset="UTF-8">
        <title><?php echo WEBNAME;?> Manage Services</title>
                ...
                <div class="row ">
                    <div class="col-12  align-self-center">
                        <div class="sub-header mt-3 py-3 px-3 align-self-center d-sm-flex w-100 rounded">
                            <div class="w-sm-100 mr-auto"><h4 class="mb-0">Manage Services</h4></div>
                                        ...
                                        <div class="col-12">
                                            <form method="post" id="home_slider1" enctype="multipart/form-data">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" class="form-control" id="name" name="name" value="<?php if(isset($service) && count($service)>0) echo $service['name'];?>" placeholder="Enter Service Name" required="">
                                                </div>
                                                <div class="form-group">
                                                    ...
                                                    <img src="../upload/services/<?php echo $service['image'];?>" class="rounded img-thumbnail hidden_cls" style="width: 40%;margin-top: 15px;">
                                                    <?php } ?>
                                                </div>
                                                <div class="form-group">
                                                    <label for="description">short Description</label>                                                  
                                                    <textarea class="form-control" id="description" name="description" placeholder="Enter Service description (Max 100 character)" maxlength="100" required=""><?php if(isset($service) && count($service)>0) echo $service['description'];?></textarea>
                                                </div>
                                                <div class="form-group">                                                  
                                    ...
                                    <table id="example" class="display table dataTable table-striped table-bordered" >
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td width="30%"><img src="../upload/services/<?php echo $ser['image'];?>" style="width: 100%;" class="img-thumbnail"></td>
                                                <td width="25%"><?php echo $ser['description'];?></td>
                                                <td width="15%">
                                                    <a href="services.php?sid=<?php echo $ser['id'];?>&image=<?php echo $ser['image'];?>"><i class="far h5 mr-2 fa-edit"></i></a>
                                                    <a href="services.php?did=<?php echo $ser['id'];?>&image=<?php echo $ser['image'];?>" onclick="return confirm('Are you sure you want to delete this item?');"><i class="fas h5 mr-2 fa-times"></i></a>
                                                </td>
                                            </tr>
                                            <?php } }?>
                                        </tbody>
            ...
            function deleteSectionImage(id,image_name){
                if (confirm("Are you sure delete this image?")) {
                   ...
                   $.ajax({
                        url:"action.php",
                        type:"POST",
                        data:'action=deleteServiceImage&id='+id+'&image_name='+image_name,
                        processData: false,
